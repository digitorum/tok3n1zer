(function (root, fabric) {
    ...
    function (States, Produce) {
        ...
        Parser.prototype.appendToBuffer = function (char) {
            ...
            // Бафер с кавычками не должен стакаться, кавычки должны идти отдельно.
            if (buffer.state == this._state && this._state != States.STATE_TEXT_QUOTE_CLOSE && this._state != States.STATE_TEXT_QUOTE_OPEN) {
                buffer.value += char;
            } else {
        ...
        Parser.prototype['state' + States.STATE_TEXT_NOT_LETTER] = function (char) {
            if (char == "&") {
                ...
                if (btype == States.STATE_TEXT_SPACE || btype == States.STATE_TEXT || btype === null) {
                    // Открывающая кавычка
                    ...
                    case '&quot;':
                        ...
                        while (matching) {
                            ...
                            if (buffer.state != States.STATE_TAG_FULL) {
                                if (buffer.state == States.STATE_TEXT_SPACE || buffer.state == States.STATE_TEXT) {
                                    this.replaceBufferType(States.STATE_TEXT_QUOTE_OPEN);
                                    matching = false;
                                } else {
                                    this.replaceBufferType(States.STATE_TEXT_QUOTE_CLOSE);
