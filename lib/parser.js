(function (root, fabric) {
    ...
    function (States, Produce) {
        ...
        var CAP = /[A-Z\u0400-\u042F]/;             // Заглавные буквы
        var Q = /["«»„“]/;                          // Кавычки
        var END = /[\.\?!]/;                        // Символы, которые могут заканчивать предложение
        var ABBR = null;                            // Символы с которых начинаются сокращения
        ...
        Parser.prototype.setAbbreviations = function (list) {
            ...
            // Плохо!
            // Потроха класса не должны влиять на глобальные переменные.
            // TODO: переделать.
            ABBR = new RegExp("[" + letters.join("") + "]", "i");
            return this;
        }
        ...
        Parser.prototype['state' + States.STATE_TEXT_SENTENCE_END] = function (char) {
            ...
            if (this.getBufferVal() == ".") {
                // Значение текущего буфера равно точке, вроверяем длину предыдущего буфера и регистр.
                var prev = this.getLastWithType(2, States.STATE_TEXT_WORD, function (buff) {
                ...
                if (prev) {
                    ...
                    if (prev.length == 1 && CAP.test(prev)) {
                        // Заменяем одну заглавную букву с точкой как {STATE_ABBREVIATION_NAME}.
                        this.replaceBuffersType(2, States.STATE_ABBREVIATION_NAME, function (buff) {
                            return buff.state != States.STATE_TAG_FULL && buff.state != States.STATE_TEXT_SPACE;
                        });
                    }
                }
