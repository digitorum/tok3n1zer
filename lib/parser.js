(function (root, fabric) {
    ...
    function (States, Produce) {
        ...
        Parser.prototype['state' + States.STATE_TEXT_NOT_LETTER] = function (char) {
            if (char == "&") {
                // Амперсанд
                return { code: States.STATE_TEXT_HTML_ENTITY, backward: 1 };
            } else if (char == "<") {
        ...
        /**
         * Обработчик состояния States.STATE_TEXT_HTML_ENTITY
         *
         * @param char
         * @returns {object}
         */
        Parser.prototype['state' + States.STATE_TEXT_HTML_ENTITY] = function (char) {
            if (char == '&' || char == "<" || SP.test(char)) {
                ...
                switch (this.getBufferVal()) {
                    ...
                    case '&nbsp;':
                        ...
                        break;
                    case '&quot;':
                        // Обычная кавычка, которую надо проанализировать на предмет принадлежности к "открывающим" или "закрывающим"
                        var matching = true;
                        var shift = 2;
                        ...
                        while (matching) {
                            ...
                            if (buffer.state != States.STATE_TAG_FULL) {
                                if (buffer.state == States.STATE_TEXT_SPACE) {
                                    this.replaceBufferType(States.STATE_TEXT_QUOTE_OPEN);
                                    matching = false;
                                } else {
                                    this.replaceBufferType(States.STATE_TEXT_QUOTE_CLOSE);
                ...
                return { code: States.STATE_TEXT_NOT_LETTER };
            } else if (EL.test(char)) {
                this.appendToBuffer(char);
            } else {
                throw "States.STATE_TEXT_HTML_ENTITY: Unexpected char `" + char + "`";
            }
            return { code: States.STATE_TEXT_HTML_ENTITY };
        }
        ...
        Parser.prototype.parseString = function (str) {
            ...
            return new Produce(this._buffers);
        }
